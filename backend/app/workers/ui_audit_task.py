...
async def _run_ui_audit(test_id: str):
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=300.0) as client:
                response = await client.post(
                    f"{settings.NODE_WORKER_URL}/midscene/run",
            ...
            summary = result_data.get("summary", f"UI audit completed: {passed_steps}/{total_steps} checks passed.")
...
@celery_app.task(name="app.workers.ui_audit_task.run_ui_audit", bind=True, max_retries=1)
def run_ui_audit(self, test_id: str):
    """Celery task to run a UI audit using Midscene.js via the node sidecar."""
    try:
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        loop.run_until_complete(_run_ui_audit(test_id))
        loop.close()
